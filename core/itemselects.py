...
def decfullquarter(l, game: GameHead):
    pl1 = game.pnum1
    pl2 = game.pnum2
    if l[0] < 128:
        return [pl1[l[0]], l[1]]
...
class PlayerGameSelect(AllSelect):
    ...
    def checkline(self, psl: PlayerStatLine) -> bool:
        ...
        if result:
            self.outputlist = decplayerline(psl.stats, self.game)
        return result
    ...
    def lineoutput(self, psl: PlayerStatLine):
        return decplayerline(psl.stats, self.game)

    def fqoutput(self, fq):
        return decfullquarter(fq, self.game)
...
class GameSelect(AllSelect):
    ...
    def output(self):
        return [decgame(self.game)] + [decplayerline(i, self.game) for i in self.gameoutputlist]
